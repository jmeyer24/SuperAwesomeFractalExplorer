...
function init() {
    ...
    gui.add( scene_params, "width segments").min(1).max(50).step(1).onChange(function ( number ) {
        widthSegments_sphere_perVertex = number;
    })
    // add number height segments
    gui.add( scene_params, "height segments").min(1).max(50).step(1).onChange( function ( number ) {
